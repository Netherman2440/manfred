...
def build_list_cities_tool(negotiations_service: BaseNegotiationsService) -> Tool:
    async def handle(
        args: dict[str, Any],
        ...
        try:
            matches = negotiations_service.search_cities(raw_query, limit=limit)
    ...
    description = (
        "Search the city catalog by substring. Returns up to `limit` exact city names "
        "(case-insensitive substring match). Use this to resolve a user's natural-language "
        "mention of a city (e.g. 'stolicy', 'warszawka') to the EXACT name expected by "
        "`get_items_for_city`. Returns only `names` — pick one and pass it back verbatim."
    )
        ...
        definition=FunctionToolDefinition(
            ...
            parameters={
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Substring to match against city names (case-insensitive).",
                    },
                    "limit": {
                        "type": "integer",
                        "minimum": 1,
